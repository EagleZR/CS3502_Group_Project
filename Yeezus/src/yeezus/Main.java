...
public class Main {
	...
	public Main( int numCPUs, CPUSchedulingPolicy policy, int diskSize, int ramSize, int registerSize ) {
		...
		try {
			// Initialize memory
			disk = new Memory( diskSize );
			MMU mmu = new MMU( new Memory( ramSize ) );
			Memory registers = new Memory( registerSize );
			...
			Driver.loadFile( disk, new File( ( URLDecoder.decode(
					...
					"UTF-8" ) ) ) );
			driver = new Driver( 0, disk, mmu, registers, policy );
		} catch ( Exception e ) {
	...
	private void excelPrint( Driver driver ) throws Exception {
		...
		for ( int i = 1; i <= TaskManager.INSTANCE.getPCBs().size(); i++ ) {
			// Write to cells
